...
public class NinePuzzle{
	...
	*/
	...
	public static boolean SolveNinePuzzle(int[][] B){	
		if (!checkB(B)){
			System.out.println("ERROR: Input Board");
				System.out.println("Input Boards must follow these guidelines:");
				System.out.println("\t- Must be a 3x3 board");
				System.out.println("\t- Must contain Integers 0 - 8");
				System.out.println("\t- Must NOT contain duplicate values");
				System.out.println("Please try again");
			return false;
		}
		LinkedList<Integer>[] G = buildGraph();
	...
	public static boolean checkB(int[][] B){
		...
		for (int i=0; i<3; i++){
			for (int j=0; j<3; j++){
				if (B[i][j] > 8 || B[i][j] < 0 || mark[B[i][j]] == true)
					return false;
				mark[B[i][j]] = true;
			}
		}
	...
	public static boolean pathTo(int v){
		if (!marked[v])
			return false;
		...
		path.push(x);
		printBoard(getBoardFromIndex(x));
		printBoard(getBoardFromIndex(edgeTo[x]));
		return true;
	}

	public static void bfs(LinkedList<Integer>[] G){	
	...
	public static int[] emptySpace(int[][] B){
		int[] ij = {-1,-1};
		for (int i=0; i<3; i++){
			for (int j=0; j<3; j++){
				if (B[i][j] == 0){
	...
	public static LinkedList<Integer>[] buildGraph(){
		@SuppressWarnings("unchecked")
		LinkedList<Integer>[] G = new LinkedList[NUM_BOARDS];
		int n = 3;
		...
		for (int a=0; a<NUM_BOARDS; a++){
			...
			int[][] b1 = getBoardFromIndex(a);
			int[][] b2 = getBoardFromIndex(a);
			int[][] b3 = getBoardFromIndex(a);
			int[][] b4 = getBoardFromIndex(a);
			
			int[] ij = emptySpace(b1);

			if(ij[0] == -1 || ij[0] == -1){
				System.out.println("ERROR: Locating Empty Space");
				return G;				
			}
